<div id="modalEditUsuarios" class="py-12 bg-gray-700 transition duration-150 ease-in-out bg-opacity-50 z-10 absolute top-0 right-0 bottom-0 left-0 hidden"
    >
    <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
        <div class="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">

            <h1 class="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">Editar Usuario</h1>

            <form id="editarUsuario{{ $usuario->id_empleado }}" method="POST" 
                 action="{{ route('administracion.editarUsuario', $usuario->id_empleado) }}">
                @csrf
                @method('PUT')
                <div class="flex mb-4">
                    <div class="w-1/2 pr-3">
                        <label for="nombre" class="block text-gray-600 text-lg font-bold leading-tight">Nombre</label>
                ...
                <div class="flex mb-4">
                    <div class="w-1/2 pr-3">
                        <label for="curp" class="block text-gray-600 text-lg font-bold leading-tight">CURP</label>
                        <input id="curp" name="curp"
                            class="w-full mt-1 text-gray-600 focus:outline-none focus:border-indigo-700 border-gray-300 rounded border"
                            placeholder="CURP" value="{{ $usuario->curp }}" />
                    </div>
                    <div class="w-1/2 pl-3">
                        <label for="rfc" class="block text-gray-600 text-lg font-bold leading-tight">RFC</label>
                ...
                <div class="flex mb-4">
                    <div class="w-1/2 pr-3">
                        <label for="puesto" class="block text-gray-600 text-lg font-bold leading-tight">Puesto</label>
                        <input id="puesto" name="puesto"
                            class="w-full mt-1 text-gray-600 focus:outline-none focus:border-indigo-700 border-gray-300 rounded border"
                            type="text" placeholder="Puesto" value="{{ $usuario->puesto }}"/>
                    </div>
                    <div class="w-1/2 pl-3">
                        <label for="correo" class="block text-gray-600 text-lg font-bold leading-tight">Correo</label>
                ...
                <div class="flex items-center justify-start w-full mt-4">
                    <button type="submit"
                        class="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 transition duration-150 ease-in-out hover:bg-indigo-600 bg-indigo-700 rounded text-white px-8 py-2 text-md">Actualizar</button>
                    <a  id="closeModalEditUsuarios"
